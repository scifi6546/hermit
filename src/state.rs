...
impl State {
    ...
    pub fn edit_videodata(
        &mut self,
        token: String,
        to_change: VideoEditStruct,
    ) -> Result<String, String> {
        if self.is_auth(token) {
            return self
                .video_db
                .edit_video_data_path(to_change.path, to_change.data);
        } else {
            return Err("not authorized".to_string());
        }
    }
    ...
    pub fn get_vid_html_from_path(
        ...
        if self.is_auth(user_token) {
            return self.video_db.get_vid_html_from_path(
                "/videos/".to_string(),
    ...
    pub fn get_playlist_all(
        ...
        if self.is_auth(user_token) {
            return Ok(self
                .video_db
                .get_playlist_all("/videos/".to_string(), "/thumbnails/".to_string()));
        } else {
            return Err("not authorized".to_string());
        }
    }
    ...
    fn set_thumb_res(&mut self, thumb_res: u32) -> Result<String, String> {
        ...
        let video_res = videos::new(
            self.config_file.videos.video_path.clone(),
    ...
    pub fn reload_server(&mut self, video_dir: String, thumb_res: u32) -> Result<String, String> {
...
pub fn run_webserver(state_in: State, use_ssl: bool) {
    ...
    if use_ssl {
        ...
        http_server.bind("0.0.0.0:8088").unwrap().run().unwrap();
    }
}
...
fn login(
    info: web::Json<UserReq>,
    data: web::Data<RwLock<State>>,
    session: Session,
) -> Result<String> {
...
pub fn get_users(data: web::Data<RwLock<State>>, session: Session) -> impl Responder {
    ...
    if token.is_ok() {
        ...
        if out.is_ok() {
            let body = serde_json::to_string(&UsersApi {
                users: out.unwrap(),
            });
            if body.is_ok() {
...
pub fn api_is_setup(data: web::Data<RwLock<State>>, _session: Session) -> impl Responder {
    ...
    if state_data.is_setup() {
        let setup = IsSetupStruct {
            is_setup: "true".to_string(),
        };
        return HttpResponse::Ok().body(serde_json::to_string(&setup).unwrap());
    } else {
        let setup = IsSetupStruct {
            is_setup: "false".to_string(),
        };
        return HttpResponse::Ok().body(serde_json::to_string(&setup).unwrap());
    }
}
...
pub fn settings_api(
    ...
    if info.action == "set_resolution".to_string() {
        ...
        if temp_res.is_ok() {
            ...
            if token_res.is_ok() {
                let final_res =
                    state.set_thumb_res_auth(token_res.unwrap().unwrap(), temp_res.unwrap());
                if final_res.is_ok() {
                    return Ok("sucess".to_string());
                } else {
...
fn api_setup(
    info: web::Json<SetupStruct>,
    data: web::Data<RwLock<State>>,
    _session: Session,
) -> Result<String> {
    let mut state_data = data.write().unwrap();
    let res = state_data.setup(
        info.video_dir.clone(),
        info.username.clone(),
        info.password.clone(),
        info.thumb_res,
    );
