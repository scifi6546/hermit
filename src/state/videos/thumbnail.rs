...
pub fn make_thumb(
    ...
    if !vid_dir_path.is_file() {
        return Err(format!(
            "thumbnail.rs: vid_dir {} is not a file",
            vid_dir_path.display()
        ));
    }
    ...
    if !thumb_dir_path.is_dir() {
        return Err(format!(
            "thumbnail.rs: thumb_dir_path {} is not a directory",
            vid_dir_path.display()
        ));
    }

    //getting video name
    let mut vid_name: String = vid_dir_path
        .file_name()
        .unwrap()
        .to_str()
        .unwrap()
    ...
    let thumb_comand = Command::new("ffmpegthumbnailer")
        .args(&[
            "-s",
            resolution.to_string().as_str(),
            "-i",
            vid_dir_path.to_str().unwrap(),
            "-o",
            thumb_path_final.to_str().unwrap(),
        ])
        .output();
